...
class GenerationRequest(models.Model):
    ...
    via the AI engine (Suno/MusicGPT).
    ...
    # SRS FR2.1: Song Title (text input)
    song_title = models.CharField(
        max_length=255,
        blank=True,
        default="",
        help_text="Optional song title supplied by the user (SRS FR2.1)"
    )
    ...
    def __str__(self):
        title_part = f" '{self.song_title}'" if self.song_title else ""
        return (f"GenerationRequest({self.occasion}/{self.mood}/{self.genre}"
                f"{title_part} @ {self.requested_duration_seconds}s by {self.owner.email})")
