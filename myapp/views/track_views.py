...
def track_list(request):
    ...
    GET  /api/tracks/?q=title  — search tracks by title (FR4.5)
    POST /api/tracks/          — create a track manually
    """
    if request.method == 'GET':
        qs = Track.objects.select_related('owner', 'generation_request').order_by('-created_at')

        # FR4.5 — search by song title
        q = request.GET.get('q', '').strip()
        if q:
            qs = qs.filter(title__icontains=q)

        # FR4.3 / FR4.4 — filter by owner if owner_id provided
...
def track_detail(request, pk):
    ...
    DELETE /api/tracks/<id>/  — delete track (FR4.6 / FR4.7)
    """
    ...
    # DELETE — permanently removes track (FR4.6, FR4.7)
    track.delete()
